Общее понимание всеми разработчиками принятого командой code style не менее важно, чем общая точка зрения на архитектуру приложения и ответственности разных объектов. Наличие соглашений по работе с VIPER-стеком - не исключение.
...
##### Описание
В случае, если в интеракторах нескольких модулей есть **повторяющаяся логика по использованию сервисов** определенным образом, она инкарпсулируется в отдельный фасад над сервисами. 
...
- Если в данном модуле нам нужно самостоятельно решать, в какой момент просить данные из кеша, а в какой - из сети, допустимо явно указывать это интерактору (`obtainFromNetwork/-fromCache`).
...
- Все `IBOutlet`'ы и `IBAction`'ы (то есть все зависимости и интерфейс) View выносятся в ее публичный интерфейс.
...
- **Комментарии к интерфейсам неуникальных компонентов** (интерактор, презентер, view), а также к их протоколам, **не пишутся**. Единственный комментарий остается в интерфейсе Assembly, как компонента, описывающего устройство рассматриваемого модуля. В этом комментарии описывается предназначени всего модуля.
